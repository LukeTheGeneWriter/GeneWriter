...
- **GPU batched path is wired into the actual GA loop** (`ga.py`'s `seed_population()`/`refresh_change_vectors(..., xp=...)`, threaded through `run_ga(..., xp=...)` and `schedule.run_schedule(..., xp=...)` / `ScheduleContext.xp`): every exact, whole-population computation — initial seeding (`run_ga`'s seed list, schedule.py's `"input"` step) and every exact refresh (`"kill_off"`/`"select"`/`"flatten"`, and `run_ga`'s periodic `refresh_every`) — now routes through `batch_calculate_change_vectors()` when `xp=numpy`/`xp=cupy` is passed, instead of a Python loop calling `calculate_change_vector()` once per individual. `xp=None` (default) keeps the original per-individual path untouched — this is opt-in, not a behavior change to existing callers. `"growth"`'s own new-genotype scoring is deliberately unaffected either way: it's cheap parent-diffing (`diff_change_vector`), not a batching target — see its docstring. Verified end-to-end on real GPU hardware: `run_ga`/`run_schedule` with `xp=cupy` land on the identical final population (same genotypes, same counts) as the default per-individual path given the same RNG seed (`tests/test_ga_schedule_gpu_wiring.py`).
...
- **Kmer batching**: not implemented. Its substring→score lookup is a hash-map operation over arbitrary-length k-mer strings, not a natural fit for array vectorization without re-encoding k-mers as base-4 integers first. `batch_calculate_change_vectors()` currently falls back to the per-individual (already-vectorized, but not batched/GPU) implementation for this one term, which measurably dilutes the batched pipeline's overall speedup (Kmer was ~34% of per-individual runtime in one real-gene benchmark) — this now also dilutes `xp=cupy` runs of the actual GA loop (see §3), not just the standalone `gpu_change_vector.py` benchmark.
- **`"growth"`'s own new-genotype scoring is still per-individual, by design, not an oversight.** Only the whole-population exact recomputes (seeding, kill_off/select/flatten's refresh) route through `xp` — see §3. Batching growth's diffing would mean giving up the cheap parent-diff approximation entirely (a genuinely new genotype has no batch of "siblings" to diff against at the moment it's created), which is a different, larger design question than the one this session's wiring answered — not attempted here. **This turned out to matter a lot more than it sounded like it would**: growth (specifically `directed_evolution`'s lookahead scoring) is ~60-85x slower per individual than the batched refresh path — see §6 — which is almost certainly the real explanation for multi-hour real-scale runs (a real Colab run and a local repro both silently hung past an hour before this was found). Batching this is the actual next real-scale bottleneck, not Kmer.
- **Progress instrumentation added** (`ga.run_ga(progress=, progress_every=)`, `schedule.run_schedule(progress=, progress_every=)`, `gpu_change_vector.batch_calculate_change_vectors(progress_every=)`): diagnostic-only prints (per-generation/per-step timing, plus every-N-individuals throughput within growth/refresh/seeding loops), all default off/silent so existing behavior and tests are unaffected. Added specifically because a long real-scale run gives *zero* feedback otherwise — this is what surfaced the growth bottleneck above; use it before assuming any future slow run is a bug rather than growth's known cost.
...
- **Real-scale (tens of thousands of individuals) has not been run end-to-end.** The GPU batching was benchmarked up to population=10,000 on the 4 batchable terms in isolation (see §6) — a full schedule run (growth + selection + Kmer) at that scale has not been attempted; growth's unbatched lookahead cost (see above), not Kmer, would now be expected to dominate.
...
- **"growth" (`directed_evolution`'s lookahead), not anything the xp/GPU wiring touches, is the real per-generation bottleneck at real scale** — found via the `progress`/`progress_every` instrumentation added to `ga.run_ga()`/`schedule.py` (see §5) after a real Colab run and a local repro both silently ran for over an hour with no visibility into where the time was going. Measured on a real gene (59 residues, small end of real data) on this RTX 3050:
  - growth (lookahead=True, the default): **~20-27 individuals/sec**, both in `run_ga` and `schedule.py`'s "growth" step.
  - the exact refresh path it's compared against: ~350/sec per-individual (`xp=None`), ~1,600/sec batched (`xp=numpy`, Kmer-loop-bound) on the same hardware/gene.
  - i.e. growth is **~60-85x slower per individual** than the already-existing batched path, and this gap is what actually explains multi-hour real-scale runs, not Kmer batching or anything `xp` touches — growth's genotypes are diffed from their parent by design (see ga.py's directed_evolution docstring), not batched, so `xp=cupy` does not speed this step up at all.
  - Root cause, not yet fixed: each of `nreplicates` (default 10) replicates scores every synonymous alternative at its chosen position via `diff_change_vector()` — many small numpy calls (~80-element excerpts) per individual, dispatch-overhead-bound rather than compute-bound at that array size. `lookahead=False` skips this entirely (see directed_evolution's docstring) as an immediate mitigation, at the cost of no longer picking the best alternative at the mutated position. Batching this scoring across individuals/replicates/alternatives the way `gpu_change_vector.py` batched the exact-refresh path is the natural real fix and is NOT started.
